...
function the_breadcrumb(){
  /* === OPTIONS === */
	...
	if (is_home() || is_front_page()) {
		if ($showOnHome == 1) $output .='<ol class="breadcrumb"><li><a href="' . $homeLink . '">' . $text['home'] . '</li></a></ol>';
	} else {
		$output .='<ol class="breadcrumb" xmlns:v="http://rdf.data-vocabulary.org/#">' . sprintf($link, $homeLink, $text['home']) . $delimiter;
...
//			$output .=$before . sprintf($text['category'], single_cat_title('', false)) . $after;
			if (single_cat_title('', false) == 'Latest News') {
				
				$output .=$before . '<a href="'. home_url().'/news-events">'.'News & Events' . "</a>". $after;
			...
			if ( get_post_type() != 'post' ) {
				...
				if (get_post_type() !== 'csr') {
				$output .=$delimiter . $before . '<a href="'. $homeLink . '' . $slug['slug'].'">' .$post_type->labels->singular_name .'</a>'. $after;
				} else {
					$output .=$delimiter . $before . '<a href="'. $homeLink . 'corporate-social-responsibility">Corporate Social Responsibility</a>'. $after;

				}
				$output .= empty( $post->post_parent ) ? '' : $delimiter . $before .  '<a href="'. get_the_permalink($post->post_parent) .'">'.get_the_title( $post->post_parent ).'</a>'. $after;
	if(get_post_type() == 'event') {
				$thetitle = truncate(get_the_title(),40);
				} else {
								$thetitle = get_the_title();
	
				}
				if ($showCurrent == 1) $output .=$delimiter . $before . $thetitle  . $after;
			} else {
				...
				if ($showCurrent == 1) $output .=$before .$thetitle . $after;
			}
