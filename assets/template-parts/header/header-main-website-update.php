...
/* }*/
 ...
 <?php if (get_option('disclaimer') == '1' && $_COOKIE['first_disclaimer'] !== '1' || $_COOKIE['second_disclaimer'] !== '1' && get_option('disclaimer') == '1' ) { echo "<p class='navbar-text'>Please complete the disclaimer below</p>";
	 	 ...
	 	 $alt2text = 'HBJ Gateley';
			 $alttext = 'Gateley Plc';
			  ...
			  $localt2text = 'Scotland <small>- for more information on HBJ Gateley click here</small>';
			 $localttext = 'England';
 ...
 if(!empty(get_theme_mod("gateley_menu_logo_link"))) {
	 ...
	 echo "</a></h5>";
                                        ...
                                        <div class="<?php   if(!preg_match('/(?i)msie [5-7]/',$_SERVER['HTTP_USER_AGENT'])){ ?>
 vc_col-sm-<?php echo $columnclass; ?> vc_col-xs-12 <?php } else { ?> columns-12<?php } ?><?php if(empty($cols)) { ?>mt20 <?php }?>">
...
if(!empty(get_theme_mod("gateley_hbj_menu_logo_link"))) {
	 echo "<h5><a href='".get_theme_mod("gateley_hbj_menu_logo_link")."'>";
           ...
           echo "<img src='".get_theme_mod("gateley_hbj_menu_logo")."' alt='". $alt2text . " Logo' class='  menu-logo ".$logoclass2."'/><span class='location-text'>". $localt2text."</span>"; } 
             if(!empty(get_theme_mod("gateley_hbj_menu_logo_link"))) {
	 echo "</a></h5>";
 }
...
<?php
					...
					if($_COOKIE['first_disclaimer'] == '1' && $_COOKIE['second_disclaimer'] == '1') {
					 ...
					 echo '<li class="gateley-menu-fw"><a href="'. network_site_url().'" ><em class="icon-home menu-icon"></em>  <span class="sep '.$mobileclasses.'">|</span><span class="'.$mobileclasses.'">Main Website</span></a></li>';
