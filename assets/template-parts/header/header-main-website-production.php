  ...
  <div class=" container">
    ...
    <div class="navbar-header">
      ...
      <?php
        ...
        if (isset($post->ID)) {
          $terms = wp_get_post_terms($post->ID, 'gateley_plc_or_hbj_gateley', array("fields" => "names"));
    ...
    <ul class="nav navbar-nav navbar-right">
      ...
      <?php
        ...
        if (get_option('disclaimer') == '1' && $_COOKIE['first_disclaimer'] !== '1' || $_COOKIE['second_disclaimer'] !== '1' && get_option('disclaimer') == '1' ) {
          echo "<p class='navbar-text'>Please complete the disclaimer below</p>";
        } else {
              ...
              <a href="#" data-toggle="dropdown" class="dropdown-toggle"><i class="icon-menu menu-icon"></i> <span class="sep">|</span> Menu</a>
                <ul class="dropdown-menu">
                  <li class="container">
                    ...
                    <div class=" <?php if(!preg_match('/(?i)msie [5-7]/',$_SERVER['HTTP_USER_AGENT'])){ ?>vc_col-sm-<?php echo $columnclass; ?> vc_col-xs-12 <?php } else { ?> columns-12<?php } ?> ">
                    ...
                    <div class="pull-left">
                      ...
                      <?php if(!empty(get_theme_mod("gateley_plc_menu_logo"))) {
                        ...
                        if(!empty(get_theme_mod("gateley_menu_logo_class"))) {
                          $logoclass = get_theme_mod("gateley_menu_logo_class");
                        } else {
                          $logoclass = '';
                        }
                  ...
                  <?php } else { ?>
	                  ...
	                  <?php if(get_theme_mod("gateley_capitas_show_menu_logo") == 1) { ?>
	                    <div class="<?php if(!preg_match('/(?i)msie [5-7]/',$_SERVER['HTTP_USER_AGENT'])){ ?>vc_col-sm-4 vc_col-xs-12 <?php } else { ?> columns-12<?php } ?><?php if(empty($cols)) { ?>mt10 <?php }?>">

	                      <?php

	                        if(!empty(get_theme_mod("gateley_capitas_menu_logo"))) {

	                          if(!empty(get_theme_mod("gateley_capitas_menu_logo_class"))) {
	                            $logoclass2 = get_theme_mod("gateley_capitas_menu_logo_class");
	                          } else {
	                            $logoclass2= '';
	                          }
	                  ...
	                  <div class="<?php   if(!preg_match('/(?i)msie [5-7]/',$_SERVER['HTTP_USER_AGENT'])){ ?>vc_col-sm-4 vc_col-xs-12 <?php } else { ?> columns-12<?php } ?><?php if(empty($cols)) { ?>mt10 <?php }?>">
	                    ...
	                    <?php
	                      ...
	                      if(!empty(get_theme_mod("gateley_hbj_menu_logo"))) {

	                        if(!empty(get_theme_mod("gateley_hbj_menu_logo_class"))) {
	                          $logoclass2 = get_theme_mod("gateley_hbj_menu_logo_class");
	                        } else {
	                          $logoclass2= '';
	                        }
            ...
            <li class="dropdown gateley-menu-fw">
              ...
              <ul class="dropdown-menu">
                <li class="container">
                  ...
                  <div class="<?php   if(!preg_match('/(?i)msie [5-7]/',$_SERVER['HTTP_USER_AGENT'])){ ?>vc_col-sm-3 vc_col-xs-12 <?php } else { ?> columns-5<?php } ?>">
                    <h5>Use this form to search for people, pages, events and our latest news.</h5>
                  </div>

                  <div class="<?php   if(!preg_match('/(?i)msie [5-7]/',$_SERVER['HTTP_USER_AGENT'])){ ?>vc_col-sm-9 vc_col-xs-12 <?php } else { ?> columns-5<?php } ?>">
                    <?php get_search_form( ) ?>
                  </div>
