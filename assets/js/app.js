	   ...
	   // This is implicit for LHS operands of the in operator   
	      // b. Let kPresent be the result of calling the   
		    // HasProperty internal method of O with argument Pk.    
		      // This step can be combined with c      // c. If kPresent is true, then    
...
if ($_GET['glossary'] !== '') {
	console.log($('#'+$_GET['glossary']).length);
$('.nav-tabs a[href="#' + $_GET['glossary'] + '"]').tab('show');
...
jQuery("#disclaimerAcceptButton").hide();
jQuery("input[name=ban_radio]").change( function(){
		var CT = jQuery(this).val();
		var CTY = jQuery("#country").val();
		console.log(CTY + " + " + CT);
		
		if(CT == 1 && CTY == "UK") {

			 jQuery("#disclaimerAcceptButton").fadeIn();
		
		} else {
						 jQuery("#disclaimerAcceptButton").fadeOut();

		}
		
	});
	jQuery("#country").change( function(){
		var CT = jQuery("input[name=ban_radio]:checked").val();
		var CTY = jQuery("#country").val();
		console.log(CTY);
...
jQuery(document).ready(function(){
console.log(jQuery.urlParam('scrollto'));  
});

jQuery(window).load(function() {  jQuery("body").addClass("loaded"); });
